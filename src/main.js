...
function parseAhkScript(rawText) {
  ...
  while (i < lines.length) {
    ...
    if (hotstringMatch) {
      ...
      const returnLine = (lines[i + 2] || "").trim();
      ...
      return `
        <li class="hotkey-item${editingClass}" data-index="${index}">
          <div class="hotkey-item-main">
            ...
            <span class="hotkey-desc">${actionLabel}: <strong>${escapeHtml(hk.actionValue)}</strong></span>
...
function handleAddOrSaveHotkey() {
  ...
  const newEntry = { prefix, actionType: type, actionValue: value, comment };
